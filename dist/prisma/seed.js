...
const getSaltRounds = () => {
    const value = Number(requireEnv(config.bcrypt_salt_rounds, "BCRYPT_SALT_ROUNDS"));
...
const getDefaultDepartment = async () => prisma.department.upsert({
    where: { code: "CSE" },
    update: {},
    create: {
...
const getDefaultSemester = async () => prisma.semester.upsert({
...
export const seedSuperAdmin = async () => {
    const email = requireEnv(config.super_admin.email, "SUPER_ADMIN_EMAIL").toLowerCase();
    ...
    const password = requireEnv(config.super_admin.password, "SUPER_ADMIN_PASSWORD");
...
export const seedTesterAdmin = async () => {
    const email = requireEnv(config.tester_admin.email, "TESTER_ADMIN_EMAIL").toLowerCase();
    const existingUser = await prisma.user.findUnique({ where: { email } });
    if (existingUser) {
    ...
    const password = requireEnv(config.tester_admin.password, "TESTER_ADMIN_PASSWORD");
    const hashedPassword = await hashPassword(password);
    const user = await prisma.user.create({
        data: {
            email,
...
export const seedTesterFaculty = async () => {
    const email = requireEnv(config.tester_faculty.email, "TESTER_FACULTY_EMAIL").toLowerCase();
    const existingUser = await prisma.user.findUnique({ where: { email } });
    if (existingUser) {
    ...
    const password = requireEnv(config.tester_faculty.password, "TESTER_FACULTY_PASSWORD");
...
export const seedTesterStudent = async () => {
    const email = requireEnv(config.tester_student.email, "TESTER_STUDENT_EMAIL").toLowerCase();
    const existingUser = await prisma.user.findUnique({ where: { email } });
    if (existingUser) {
    ...
    const password = requireEnv(config.tester_student.password, "TESTER_STUDENT_PASSWORD");
